...
contract('Token', ([deployer, receiver, exchange]) => {
    ...
    beforeEach( async () => {
        token = await Token.new(_name, _symbol, _decimals, _totalSupply, {from: deployer, gasPrice: 0})
    })
    ...
    describe('sending tokens', () => {
        ...
        describe('success', () => {
            ...
            /*
             -needs checking -events givign different outputs on OVM
    ...
    describe('approving tokens', () => {
        ...
        beforeEach(async () => {
          amount =  tokenFormat(100)
          resultApprove = await token.approve(exchange, amount, { from: deployer, gasPrice: 0})
        })
        ...
        describe('success', () => {
            ...
            /*
             - needs checking event giving differnt outputs on OVM
    ...
    describe('delegated token transfers', () => {
        ...
        beforeEach(async () => {
          amount = tokenFormat(100)
          let c = await token.approve(exchange, amount, { from: deployer, gasPrice: 0 })
        })
    
        describe('success', () => {
            beforeEach(async () => {
                result = await token.transferFrom(deployer, receiver, amount, { from: exchange, gasPrice: 0 })
            })
            ...
            /*
             - needs to be checked - events giving different outputs on OVM
            it('emits a Transfer event', () => {
                // Get event logs check all correct our emitted event is ...
                // event Transfer(address indexed from, address indexed to, uint256 value);
                const log = result.logs[0]
        ...
        describe('failure', () => {
            ...
            it('rejects invalid recipients', () => {
                token.transferFrom(deployer, 0x0, amount, { from: exchange, gasPrice: 0}).should.be.rejected
            })
        })
    })
